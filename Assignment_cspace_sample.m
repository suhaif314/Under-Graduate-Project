...
xcircle = -0.5 + 0.5*cosd(th_c);
ycircle = 0.5+0.5*sind(th_c);
...
for theta_1 = th1:5:360+th1         %for the rotation of the link 1
    for theta_2 = th2:5:360+th2     %for the rotation of the link2
        ...
        X2 = x2+L2/2*cosd(theta_1+theta_2);  % xcord of the middle of the link2
        Y2 = y2+L2/2*sind(theta_1+theta_2);  % ycord of the middle of the link2

        link_1 = polyshape(X1+link_1_updated(:,1),Y1+link_1_updated(:,2)); %form a complete link1 
        link_2 = polyshape(X2+link_2_updated(:,1),Y2+link_2_updated(:,2)); %form a complete link2
        ...
        anime1 = animatedline('Marker','.','Color','black');
        anime2 = animatedline('Marker','.','Color','blue');
        anime3 = animatedline('Marker','.','Color','yellow');
        anime4 = animatedline('Marker','.','Color','red');

        if (overlaps(link_1,ob_1)||overlaps(link_2,ob_1))
